    ...
    if (isset($_SESSION['idAdmon'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'updateStock':
                ...
                break;
                //Caso para obtener el historial de un inventario
            case 'inventoryHistory':
                if ($inventario->setId($_POST['id_inventario'])) {
                    if ($result['dataset'] = $inventario->inventoryHistory()) {
                        $result['message'] = 'Registros obtenidos';
                        $result['status'] = 1;
                    } else {
                        if (Database::getException()) {
                            $result['exception'] = Database::getException();
                        } else {
                            $result['exception'] = 'No hay historial de este producto.';
                        }
                    }
                } else {
                    $result['exception'] = 'Id incorrecto';
